...
namespace RektorlukDers.Class
{
    public class DbProcess
    {
        ...
        //SqlConnection con = new SqlConnection("Data Source=localhost;Initial Catalog=otobus_firmasi;integrated Security=SSPI");
        int result;
        ...
        public int IletisimIDGetir(int Tip, int TipID)
        {
            con.Open();
            SqlCommand cmd = new SqlCommand("SELECT ID FROM Iletisim where Tip = " + Tip + " and  TipID= " + TipID + "", con);
            SqlDataReader dr = cmd.ExecuteReader();
            while (dr.Read())
            {
                result = int.Parse(dr["ID"].ToString());
        ...
        public void PersonelGuncelle(Personel inPersonel, Firma inFirma)
        {
            ////PersonelIDGetir(inPersonel.ID);
            //Sorgu("update Personel set Maas='" + inPersonel.maas + "',Vardiya='" + inPersonel.vardiya + "',Gorev='" + inPersonel.gorevi + "',InsanID='" + InsanIDGetir(inPersonel.insan.tc) + "',FirmaID='" + FirmaIDGetir(inFirma.ad) + "' where InsanID=" + InsanIDGetir(inPersonel.insan.tc) + "");
        }
        #endregion
        ...
        public void AracGuncelle(Arac inArac, Firma inFirma)
        {
            //Sorgu("update Arac set KoltukSayisi=" + inArac.koltukSayisi + ",PlakaNo='" + inArac.plakaNo + "',Model='" + inArac.model + "',Marka='" + inArac.marka + "',FirmaID=" + FirmaIDGetir(inFirma.ad) + " where ID='" + AracIDGetir(inArac.plakaNo) + "'");
        }
        #endregion
        ...
        {
            //Sorgu("update Firma set Ad='" + inFirma.ad + "',Adres='" + inFirma.adres + "',VergiNo='" + inFirma.vergiNo + "',Fax='" + inFirma.fax + "' where ID= " + FirmaIDGetir(inFirma.ad) + "");
        }
        #endregion
        ...
        {
            ////FirmaIDGetir(inFirma.ad);
            //Sorgu("update Kampanya set IndirimYuzdesi=" + kampanya.indirimYuzdesi + ",Ad='" + kampanya.indirimAdi + "',firmaID='" + kampanya.firma.ID + "' where ID='" + KampanyaIDGetir(kampanya.indirimAdi) + "'");
        }
        #endregion
        ...
        public void IkramEkle(Ikram inIkram, Arac inArac)
        {
            Sorgu("Insert Into Ikram (IkramAdi,Adet,Tur,AracID) Values ('" + inIkram.adi + "'," + inIkram.adedi + ",'" + inIkram.turu + "'," + AracIDGetir(inArac.plakaNo) + ")");
        }

        public void IkramGuncelle(Ikram inIkram, Arac inArac)
        {
            //Sorgu("update Ikram set IkramAdi='" + inIkram.adi + "',Adet='" + inIkram.adedi + "',Tur='" + inIkram.turu + "',AracID='" + AracIDGetir(inArac.plakaNo) + "' where  ID=" + IkramIDGetir(inIkram.adi) + "");
        }
        #endregion
        ...
        {
            Sorgu("Insert Into Insan (Ad,Soyad,Tc,DogumTarihi,DogumYeri,Cinsiyet) Values ('" + inInsan.ad + "','" + inInsan.soyad + "','" + inInsan.tc + "','" + inInsan.dogumTarihi + "','" + inInsan.dogumYeri + "','" + inInsan.cinsiyet + "')");
        }

        public void InsanGuncelle(Insan inInsan)
        {
            //Sorgu("update Insan set Ad='" + inInsan.ad + "',Soyad='" + inInsan.soyad + "',Tc=" + inInsan.tc + ",DogumTarihi='" + inInsan.dogumTarihi + "',DogumYeri='" + inInsan.dogumYeri + "',Cinsiyet='" + inInsan.cinsiyet + "'  where ID='" + InsanIDGetir(inInsan.tc) + "'");
        }
        #endregion
        ...
        {
            //Sorgu("update Tip set TipAd='" + inTip.TipAd + "' where ID='" + TipIDGetir(inTip.TipAd) + "'");
        }
        #endregion
        ...
        {
            ////SeferIDGetir();
            //Sorgu("update Sefer set SeferNo=" + inSefer.seferNo + ",PeronNo=" + inSefer.peronNo + ",KalkısYeri='" + inSefer.kalkısYeri + "',VarısYeri='" + inSefer.varısYeri + "',Tarih='" + inSefer.tarih + "',Saat='" + inSefer.saat + "',Ucret=" + inSefer.ucret + ",FirmaID=" + FirmaIDGetir(inSefer.firma.ad) + ",AracID=" + AracIDGetir(inSefer.arac.plakaNo) + " where ID=" + SeferID + "");
        }
        #endregion
        ...
        {
            Sorgu("update  Bilet set Aktif=0 where ID= '" + silinecekBiletID + "'");
        }
        ...
        public void YolcuGuncelle(Insan insanID, Firma FirmaID)
        {
            //YolcuIDGetir(YolcuID);
            //FirmaIDGetir(FirmaID);
            //InsanIDGetir(insanID);
            //Sorgu("update Yolcu set InsanID=" + InsanID + ",FirmaID='" + FirmaID + "' where ID=" + YolcuID + "");
        }
        #endregion

        #region Koltuk İşlemleri

        //public void KoltukSil(int SilinecekID)
        //{
        //    //Sorgu("exec KoltukSil '" + SilinecekID + "'");
        //    //cmd.CommandType = CommandType.StoredProcedure;
        //}
        ...
        public void KoltukGuncelle(Koltuk inKoltuk, Arac inArac, Yolcu inYolcu, Sefer inSefer)
        {
            //AracIDGetir(inArac.plakaNo);
            ////YolcuIDGetir(inYolcu.ID);
            //SeferIDGetir(inSefer.ID);
            ////InsanIDGetir(InsanID);
            //Sorgu("update Koltuk set KoltukNo='" + inKoltuk.No + "',Durum='" + inKoltuk.Durumu + "',KoltukCinsiyet=" + inKoltuk.Cinsiyet + ",YolcuID='" + YolcuID + "',AracID='" + AracID + "',SeferID='" + SeferID + "'  where ID=" + inKoltuk.ID + "");
        }
        #endregion

        //Güncelleme yap ve Silme Trigger yaz.
          
        #region Iletisim İşlemleri
        public void IletisimSil(Iletisim inIletisim)
        {
            Sorgu("update Iletisim set Aktif=0 where ID= '" + IletisimIDGetir(inIletisim.tip, inIletisim.TipID) + "'");
            //cmd.CommandType = CommandType.StoredProcedure;
        }

        public void IletisimEkle(Iletisim iniletisim)
        {
            Sorgu("Insert Into Iletisim (CepTelefonu,EvTelefonu,Email,TipID,TipID) Values ('" + iniletisim.cepTelefonu + "','" + iniletisim.evTelefonu + "','" + iniletisim.mail + "'," + iniletisim.tip + "," + iniletisim.TipID + ")");
        }
